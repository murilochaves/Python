...
def plotar(PDF, margemXInicial = 0, tamanhoVertical = 0, margemYInicial = 0, larguraEtiqueta = 0, alturaEtiqueta = 0, qtdTotalEtiquetas = 0, lista = []):
    ...
    if (qtdTotalEtiquetas <= 3):
        qtd_linhas = 1
        qtd_colunas = qtdTotalEtiquetas
        coluna_final = qtdTotalEtiquetas
    elif (qtdTotalEtiquetas > 3):
        qtd_linhas = (int(qtdTotalEtiquetas / 3))
        if ((qtdTotalEtiquetas % 3) != 0):
            qtd_linhas = qtd_linhas + 1
        qtd_colunas = 3
        coluna_final = qtdTotalEtiquetas % 3

    if (coluna_final == 0):
        coluna_final = 3

    if (qtd_linhas > 7):
        paginasDocumento = qtd_linhas / 7
