...
Modelo: PIMACO A4360 (7 linhas X 3 colunas)
Documento: A4 (21.0cm x 29.7cm)
Tamanho útil: 38,1mm x 63,5mm
Qtd. Etiquetas/Folha: 21
...
Autor: JAYME, M. C.
Última modificação: 03 de dezembro de 2018
"""
...
def constituir(arquivo = "etiqueta.pdf", pagesize = A4, idCliente = 0, margemX = 0, tamHorizontal = 0, margemY = 0, tamanhoVertical = 0, larguraEtiqueta = 0, alturaEtiqueta = 0, imprimirGrade = False):
    ...
    if (imprimirGrade != False):
        print(margemX)
        print(tamHorizontal)
        print(tamanhoVertical)
        print(larguraEtiqueta)
        print(margemY)
...
def plotar(PDF, margemXInicial = 0, tamanhoVertical = 0, margemYInicial = 0, larguraEtiqueta = 0, alturaEtiqueta = 0, qtdTotalEtiquetas = 0, lista = [], qtd_linhas_documento = 0, qtd_colunas_documento = 0):

    print('\n### Plotando ###\n')

    print('\t* Qtd. total de Etiquetas: ', qtdTotalEtiquetas)

    if (qtdTotalEtiquetas <= qtd_colunas_documento):
        qtd_linhas = 1
        qtd_colunas = qtdTotalEtiquetas
        coluna_final = qtdTotalEtiquetas
    elif (qtdTotalEtiquetas > qtd_colunas_documento):
        qtd_linhas = (int(qtdTotalEtiquetas / qtd_colunas_documento))
        if ((qtdTotalEtiquetas % qtd_colunas_documento) != 0):
            qtd_linhas = qtd_linhas + 1
        qtd_colunas = qtd_colunas_documento
        coluna_final = qtdTotalEtiquetas % qtd_colunas_documento

    if (coluna_final == 0):
        coluna_final = qtd_colunas_documento

    if (qtd_linhas > qtd_linhas_documento):
        paginasDocumento = qtd_linhas / qtd_linhas_documento
        if (paginasDocumento != int):
            paginasDocumento = (int(paginasDocumento) + 1)
    elif(qtd_linhas <= qtd_linhas_documento):
        paginasDocumento = 1
    ...
    def rodar(qtd_linhas, qtd_colunas, coluna_final, lista):
        ...
        for linha in range(qtd_linhas):
            ...
            if (aux >= (qtd_linhas_documento + 1)):
                PDF.showPage()
                PDF.setFont("Helvetica", 10)
                aux = 1

            if (linha + 1 != qtd_linhas):
                if(qtd_colunas >  qtd_colunas_documento):
                    qtd_colunas = qtd_colunas_documento
...
def main():
    ...
    print('Versao 30/nov/2018 9h39 v1')
    ...
    plotar(
        ...
        qtd_linhas_documento = linhas,
        qtd_colunas_documento = colunas
        )
